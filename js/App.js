...
window.CONTRACT = {
	address: "0x8aF2b43a248f8DCEF3585e31D8899D0733f50f7C",
...
async function get_Sha3() {
	$("#note").html(
		`<h5 class="text-warning">Hashing Your Document 😴...</h5>`
	);
	...
	if (file) {
		...
		reader.onload = async function (evt) {
			...
			window.hashedfile = await web3.utils.soliditySha3(
				evt.target.result
			);
...
function printTransactions(data) {
	...
	for (let i = 0; i < data.length; i++) {
		...
		a.href =
			`${window.CONTRACT.explore}` + "/tx/" + data[i].transactionHash;
